...
//					currentTime);
			...
			for (PowerHost host : this.<PowerHost> getHostList()) {
				...
				if(previousUtilizationOfCpu==1.04 || utilizationOfCpu==1.04) {
					int i=0;
					i++;
				}
...
//					timeFrameDatacenterEnergy);
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			for (Vm vm : host.getCompletedVms()) {
				...
				getVmList().remove(vm);
				Log.printLine(CloudSim.clock() + ":VM #" + vm.getId() + " completed and has been deallocated from host #" + host.getId());
			}
		}
...
//		Log.printLine();
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		setCloudletSubmitted(CloudSim.clock());//wang
		
		super.processCloudletSubmit(ev, ack);//wang
